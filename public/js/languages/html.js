  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    if (attributes) {
      for (attribute of attributes) {
        if (attribute["name"].substring(0, 2) === 'on' || attribute["value"].substring(0, 11).toLowerCase() === 'javascript:') elements[i].removeAttribute(attribute["name"]);
      }
    }
  }
...
function setTextareaDimensions() {
  ...
  document.getElementById('lineNumbers').style.height = document.getElementById('code').offsetHeight + 'px';
  document.getElementById('textarea').scroll();
}
